...
export default function Contact({ config }: ContactProps) {
  ...
  const getMailtoUrl = () => {
    ...
    const subject = encodeURIComponent(`Nursery Enrollment Inquiry: ${submittedSnapshot.name} (${submittedSnapshot.academicStream})`);
    const body = encodeURIComponent(`Dear Admissions Office,

I would like to submit an official enrollment inquiry for ${config.name}. Please find our application details below:

[PARENT / GUARDIAN DETAILS]
- Parent/Guardian Name: ${submittedSnapshot.name}
- Selected Class: ${submittedSnapshot.academicStream}
- Contact Email: ${submittedSnapshot.email}
- Phone Number: ${submittedSnapshot.phone}

[CHILD DETAILS & CARE NOTES]
${submittedSnapshot.message}

Please confirm receipt of our inquiry and provide details regarding enrollment availability, fees structure, and start dates.
...
${submittedSnapshot.name}`);
  ...
  const handleCopyDetails = () => {
    if (!submittedSnapshot) return;
    const plainText = `Dear Admissions Office at ${config.name},

I would like to submit an official enrollment inquiry. Please find our details below:
...
Sent via Online Admissions Portal.`;
  ...
  const mapQuery = encodeURIComponent(config.name);
      ...
      <div className="max-w-[1400px] mx-auto px-2 sm:px-4 lg:px-5">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
          ...
          <p className="text-gray-500 text-sm sm:text-base">
            Give your child a bright start. Complete the quick application form below to submit an enrollment inquiry directly to our administrative team at {config.name}.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
            ...
            {submitSuccess && submittedSnapshot ? (
              <div className="space-y-6 animate-fade-in">
                ...
                <div className="bg-emerald-50 border border-emerald-100 p-5 rounded-2xl flex items-start space-x-3.5">
                  ...
                  <div>
                    <h4 className="font-sans font-extrabold text-sm sm:text-base text-emerald-900">Enrollment Inquiry Prepared!</h4>
                    <p className="text-xs text-emerald-800/80 leading-relaxed font-light mt-1">
                      Your child's enrollment form has been formatted securely. Click the button below to send it directly to <strong>{config.contact.email}</strong>, or copy the details to email us manually.
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-white border border-gray-200/60 rounded-2xl p-5 space-y-4 shadow-inner text-xs">
                  <h5 className="font-extrabold text-[11px] uppercase tracking-wider text-gray-400 border-b border-gray-100 pb-2 flex items-center space-x-1.5">
                    ...
                    <span>Inquiry Snapshot Summary</span>
                  </h5>
                  
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 pt-1">
                    <div>
                      <div className="text-gray-400 font-bold text-[10px] uppercase">Parent / Guardian</div>
                      <div className="text-gray-850 font-bold text-xs mt-0.5">{submittedSnapshot.name}</div>
                    </div>
                    <div>
                      <div className="text-gray-400 font-bold text-[10px] uppercase">Selected Class</div>
                      <div className="text-xs font-bold text-slate-800 mt-0.5" style={{ color: config.primaryColor }}>
                        {submittedSnapshot.academicStream}
                      </div>
                    </div>
                    <div>
                    ...
                    <div>
                      <div className="text-gray-400 font-bold text-[10px] uppercase">Parent Email</div>
                      <div className="text-gray-700 font-semibold mt-0.5">{submittedSnapshot.email}</div>
                    </div>
                  </div>

                  <div className="border-t border-gray-100 pt-3">
                    <div className="text-gray-400 font-bold text-[10px] uppercase">Care Notes & Special Requests</div>
                ...
                <div className="text-center pt-2">
                  ...
                  >
                    Submit Another Enrollment Form
                  </button>
                </div>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="border-b border-gray-200/50 pb-4 mb-2">
                  ...
                  <p className="text-gray-400 text-[11px] font-light mt-1">
                    Fill in your details below to prepare your child's nursery application.
                  </p>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-gray-700 uppercase tracking-wide">Parent / Guardian Name</label>
                    <input
                      type="text"
                      required
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-gray-700 uppercase tracking-wide">Nursery Level / Class</label>
                    <select
                      required
                    ...
                    >
                      ...
                      <option value="Baby Class">Baby Class (Ages 3)</option>
                      <option value="Middle Class">Middle Class (Ages 4)</option>
                      <option value="Top Class">Top Class (Ages 5)</option>
                      <option value="Daycare">Daycare / Playgroup</option>
                    </select>
                  </div>
                </div>

                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-gray-700 uppercase tracking-wide">Child Info & Care Notes</label>
                  <textarea
                    rows={4}
                    required
                    placeholder="Enter child's full name, age, dietary needs, medical notes, or preferred start term..."
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            {/* Official Fees Structure & Care Card */}
            <div 
              className="text-white rounded-2xl p-6 shadow-md border border-white/20 space-y-3.5"
              style={{
                background: `linear-gradient(135deg, ${config.primaryColor} 0%, ${config.secondaryColor} 100%)`
              }}
            >
              <div className="flex items-center space-x-2 text-white/90 font-extrabold text-xs uppercase tracking-wider">
              ...
              <h4 className="font-bold text-sm sm:text-base text-white tracking-tight">
                {config.name} Fee Structure
              </h4>
              <p className="text-white/80 text-xs leading-relaxed font-normal">
                To find official tuition rates, meal care options, and enrollment registration for Baby, Middle, or Top Class at {config.name}, contact our administrative team directly or visit our campus office. Exact term rates cover learning materials, certified caregiving, and playground access.
              </p>
              <div className="pt-2 flex items-center space-x-2 text-[11px] text-white/90 font-semibold border-t border-white/20">
            ...
            <div className="bg-gray-50 border border-gray-100 rounded-2xl p-6 space-y-5">
              <h4 className="font-bold text-slate-900 text-sm sm:text-base uppercase tracking-wider">School Office & Contact</h4>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-start space-x-3 border-t border-gray-100 pt-4">
                  ...
                  <div>
                    <div className="text-[10px] font-bold text-gray-400 uppercase tracking-wide">Administrative Email</div>
